...
def sort_samples (df, sortabund):
    ...
    taxonomy = [name for name in header if not name[0].isdigit() if not name.startswith('barcode')]
    ...
    if sortabund == "yes":
        ...
        df = df.assign(sum=df.sum(axis=1)).sort_values(by='sum', ascending=False).drop(['sum'], axis=1)
        df = df.append(unassigned) # The frame.append method is deprecated and will be removed from pandas in a future version. Use pandas.concat instead.
